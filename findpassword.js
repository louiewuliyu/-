...
<% request.setCharacterEncoding("utf-8"); 
  ...
  { 
  try{
  String connectString = "jdbc:mysql://localhost:3306/user" + "?autoReconnect=true&useUnicode=true&characterEncoding=UTF-8";
  
  Class.forName("com.mysql.jdbc.Driver").newInstance(); 

  Connection con = DriverManager.getConnection(connectString,"root", "1234"); 

  Statement stmt = con.createStatement(); 
  fmtselect="select * from user where email='%s'";
  String sqls=String.format(fmtselect,email);
  ResultSet rs=stmt.executeQuery(sqls);
  if(rs.next()!=false)
  {
    msg="邮箱已注册，";
    String turl="login.jsp?email='%s'";
  ...
  else
  {
    ...
    ResultSet r=stmt.execute(sqlv);
    if(r.next())
    {
      String code=r.getString("code");
      if(code.equals(verify))
      {
        fmt="update user(email,name,password) values('%s', '%s','%s')"; 
        String sql = String.format(fmt,email,name,pwd); 
        ...
        {
          msg="验证码错误"；
          url=String.format(url,name,email);
          words="注册";
        }
      }
    }
  ...
  msg = "注册失败";
  String turl="register.jsp?name='%s'&&emil='%s'";
   url=String.format(url,name,email);
   words="注册";
  } 
}
